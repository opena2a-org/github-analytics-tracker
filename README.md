...
- **Every headline series is defined once.** `lib/series.js` is the registry (`downloads`, `downloadsExCrypto`, `adoption`, `adoptionExCrypto`: key, label, method, scope, definition). `summary.json` repeats the flat `total.*` fields for existing consumers and adds `series.<key>` (definition, per-source window read from the database, components, `status` of `ok` or `partial`) and `collectors.<name>` (ran / ok / error per collector for the run that wrote the file, plus GitHub discovery vs traffic coverage). A failed collector marks every series that sums it `partial`; the workflow's final `verify-run` step turns the run red after the data is committed, and also fails when the published summary is older than 48 hours. `adoption` is a mixed-window sum (package, image and model download events plus GitHub's rolling 14-day distinct cloners) and is never described as deduplicated.
