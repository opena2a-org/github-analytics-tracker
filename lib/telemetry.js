 ...
 *
 * This is what a broken ingest looks like from here, and it is the reason the
 * retrieval check above is not sufficient on its own. Registry #283 (2026-06-26)
 * moved the ingest route and CLIs silently stopped POSTing for ~7 days; the
 * adoption feed this collector reads is a different endpoint and was unaffected,
 * so it would have kept answering 200 while the counts drained. (Stated as the
 * mechanism, not an observation — this collector was written 2026-07-02, after
 * the gap, so nothing here actually watched it happen.)
 *
 * A retrieval check sees a healthy 200 and persists the zeros; only comparing
 * against the previous snapshot reveals it.
 *
