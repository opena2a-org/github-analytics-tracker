 ...
 * predates the canonical row's and must not be lost).
 *
 ...
 * has 3. Views/clones are additive daily counts, but the twin histories are NOT
 * disjoint in time: GitHub's traffic API returns a 14-day window, so the last
 * collection of the old slug and the first collections of the new one cover the
 * same days, and traffic_clones is UNIQUE(repo_id, date), not per canonical repo.
 * Measured 2026-08-25 on the committed database: 61 (canonical, date) pairs held
 * a row under both slugs, every pair with identical counts, 629 clones and 261
 * views double-counted. The earlier comment here that called the histories
 * "disjoint in time" was wrong.
 *
 ...
 *   - collapse additive traffic per (canonical repo, date): on a day both twins
 *     report, take the canonical row's value, else the MAX (canonicalTrafficSum);
 *     then sum the collapsed days;
...
// SQL fragment selecting one value per (canonical repo, date) from a traffic
// table: the canonical row's count when it has one, else the largest twin value.
// Twins report identical counts on overlapping days (measured), so MAX is the
// count itself, never a sum of both.
function collapsedDailySql(table, dateWhere) {
  return `
    SELECT t.date AS date,
           COALESCE(MAX(CASE WHEN COALESCE(r.canonical_full_name, r.full_name) = r.full_name
                             THEN t.count END),
                    MAX(t.count)) AS count
    FROM ${table} t JOIN repositories r ON r.id = t.repo_id
    WHERE COALESCE(r.canonical_full_name, r.full_name) = ?${dateWhere ? ' AND ' + dateWhere : ''}
    GROUP BY t.date`;
}

/**
 * Summed additive traffic (views or clones) for one canonical repo, collapsing
 * twin rows per date. `table` is 'traffic_views' or 'traffic_clones';
 * `dateWhere` is an optional SQL predicate over t.date with its bound params.
 */
function canonicalTrafficSum(db, table, canon, dateWhere = '', params = []) {
  if (table !== 'traffic_views' && table !== 'traffic_clones') {
    throw new Error(`canonicalTrafficSum: unsupported table ${table}`);
  }
  const row = db.prepare(
    `SELECT COALESCE(SUM(count), 0) AS total FROM (${collapsedDailySql(table, dateWhere)})`
  ).get(canon, ...params);
  return row.total;
}

/**
 * All-time views and clones across every canonical repo, twins collapsed per
 * date. Used by generate-summary.js so summary.total.clones / .views match the
 * overview API. `excludeCanonical` drops one canonical repo (the ex-CryptoServe
 * series).
 */
function canonicalTrafficTotals(db, { excludeCanonical = null } = {}) {
  const canons = db.prepare(
    'SELECT DISTINCT COALESCE(canonical_full_name, full_name) AS canon FROM repositories'
  ).all().map(r => r.canon).filter(c => c !== excludeCanonical);
  let views = 0, clones = 0;
  for (const canon of canons) {
    views += canonicalTrafficSum(db, 'traffic_views', canon);
    clones += canonicalTrafficSum(db, 'traffic_clones', canon);
  }
  return { views, clones };
}
 ...
 * row per canonical repo. Snapshot metrics come from the canonical row. Additive
 * traffic is summed across the group's rows, which is only correct when the
 * caller has already collapsed twin days (overview.js builds one row per
 * canonical repo via canonicalTrafficSum, so the sum is over a single row).
 * Rows that still carry per-slug traffic must not be fed here.
 */
function mergeByCanonical(repoStats) {
  ...
  for (const [canon, group] of groupByCanonical(repoStats)) {
    ...
    out.push({
      ...
      // Additive daily traffic, already collapsed per date by the caller.
      ...
      forks: c.forks || 0,
      archived: !!c.archived,
      aliasCount: group.length - 1 + group.reduce((s, r) => s + (r.aliasCount || 0), 0),
    });
  }
  return out;
}
